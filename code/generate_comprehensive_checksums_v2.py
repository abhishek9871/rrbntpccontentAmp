...
Updated with robust error handling for permission issues.
"""
...
class ChecksumGenerator:
    ...
    def safe_get_file_info(self, file_path):
        """Safely get file information with comprehensive error handling"""
        try:
            # Check if file exists and is readable first
            if not file_path.exists():
                return None
            
            # Try to get file stats - this might fail for permissions
            try:
                stat = file_path.stat()
            except (PermissionError, OSError) as e:
                self.failed_files.append({
                    'path': str(file_path),
                    'error': f"Permission denied accessing file stats: {str(e)}"
                })
                return None
    ...
    def discover_all_files(self):
        """Discover all files in the workspace with robust error handling"""
        print("Discovering all files in workspace...")
        file_extensions = set()
        total_size = 0
        
        # List directories explicitly and filter out problematic ones
        problematic_dirs = []
        
        try:
            for root_path in self.workspace_root.iterdir():
                if root_path.is_dir():
                    if root_path.name == "browser" and "user_data" in str(root_path):
                        print(f"Skipping browser user data directory: {root_path}")
                        continue
                    
                    try:
                        # Use os.walk instead of rglob for better control
                        for dirpath, dirnames, filenames in os.walk(root_path):
                            # Remove problematic subdirectories from walk
                            dirnames[:] = [d for d in dirnames if not (d == "user_data" and "browser" in dirpath)]
                            
                            for filename in filenames:
                                # Skip problematic files
                                if filename == "SingletonSocket" or filename.endswith(".sock"):
                                    continue
                                    
                                file_path = Path(dirpath) / filename
                                file_info = self.safe_get_file_info(file_path)
                                if file_info:
                        ...
                        print(f"Permission denied accessing {root_path}, skipping...")
                        continue
                    except Exception as e:
            ...
            print(f"Error accessing workspace root: {e}")
